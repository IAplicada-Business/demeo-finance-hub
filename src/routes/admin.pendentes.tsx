...
function PendentesPage() {
  ...
  const [clientTotalsScope, setClientTotalsScope] = useState<"full" | "page">("page");
  ...
  async function loadData(p: number) {
    ...
    const filterOpts = !showAll ? { dateFrom } : undefined;

    const txQuery = pendingTransactionsFilter(
      "id, client_id, date, description, raw_description, amount, category, status, confidence",
      undefined,
      filterOpts,
    );
    ...
    const txList = (txData ?? []) as unknown as PendingTx[];
    setTransactions(txList);

    const clientIds = [...new Set(txList.map((t) => t.client_id))];
    let totals: Record<string, number> = {};
    let totalsScope: "full" | "page" = "page";

    if (clientIds.length > 0 && clientIds.length <= 5) {
      const counts = await Promise.all(
        clientIds.map(async (cid) => {
          const { count: clientCount } = await pendingTransactionsFilter(
            "*",
            { count: "exact", head: true },
            { ...filterOpts, clientId: cid },
          );
          return [cid, clientCount ?? 0] as const;
        }),
      );
      ...
      for (const tx of txList) {
        totals[tx.client_id] = (totals[tx.client_id] ?? 0) + 1;
      }
    }
    setClientTotals(totals);
    setClientTotalsScope(totalsScope);
          ...
          const installmentFields =
            inst?.enabled && inst.total > 0 && inst.number > 0 && inst.number <= inst.total
              ? {
                  ...
                  installment_group_id: await installmentGroupId(
                    tx.client_id,
                    tx.description,
                    inst.total,
                    tx.date,
  ...
  const sortedEntries = Object.entries(grouped).sort(([, a], [, b]) =>
    sortOrder === "desc" ? b.length - a.length : a.length - b.length,
  );
        ...
        {totalCount > PAGE_SIZE && (
          <div className="flex items-center justify-between">
            <div className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
              Página {page + 1} de {Math.ceil(totalCount / PAGE_SIZE)} · {totalCount} aguardando
              aprovação no total
            </div>
            <div
            ...
            <div key={cid} className="aurora-card p-0 overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                  {(() => {
                    const selWithCategory = items.filter(
                      (t) => selected[t.id] && !!cats[t.id]?.trim(),
                    );
                ...
                <table className="w-full">
                  <thead>
                    <tr>
                      <th
                        className="px-4 py-3"
                        style={{ borderBottom: "1px solid var(--line)", width: 40 }}
                      >
                        <input
                          type="checkbox"
                                ...
                                >
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setInstallments({
                                        ...installments,
                                        [t.id]: { ...inst, enabled: e.target.checked },
                                      })
                                    }
                                  />
                                  Parcelamento
