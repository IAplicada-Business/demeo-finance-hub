...
function drawFooter(page: PDFPage, label: string, font: PDFFont) {
  ...
  page.drawText(`${AURORA_EMAIL}  ·  contato: ${CM_PHONE}`, {
    x: ML,
    y: 30,
    font,
    size: 8,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (admin) {
      if (!bodyClientId)
        return jsonResponse({ error: "client_id obrigatório para admin" }, 400, origin);
      ...
      if (!mapping?.client_id)
        return jsonResponse(
          { error: "Acesso negado: usuário não vinculado a nenhum cliente" },
          403,
          origin,
        );
    ...
    const despList = Array.from(despCat.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
    ...
    if (despList.length === 0) {
      ...
      for (const [cat, val] of despList) {
        if (y < 80) break;
        p1.drawText(cat, { x: ML, y, font, size: 10, color: rgb(0.11, 0.11, 0.1) });
    ...
    y = Math.max(y, 70);
    p1.drawText(`Gerado em ${todayBR()} · Aurora Gestão Financeira`, {
      x: ML,
      y: 58,
      font,
      size: 8,
    ...
    if (dre.groups.length > 0) {
      ...
      let dy = A4.height - 90;
      p2.drawText("DEMONSTRATIVO DE RESULTADO (DRE)", {
        x: ML,
        y: dy,
        font: fontBold,
        size: 10,
      ...
      for (const g of dre.groups) {
        ...
        for (const l of g.lines) {
          if (dy < 80) break;
          p2.drawText(l.cat, { x: ML + 16, y: dy, font, size: 9, color: rgb(0.11, 0.11, 0.1) });
        ...
        if (g.name === DRE_EBITDA_AFTER && dy > 80) {
          p2.drawRectangle({
            ...
            width: A4.width - ML - MR,
            height: 20,
            color: rgb(0.88, 0.93, 0.87),
          });
          p2.drawText("= Resultado Operacional (EBITDA)", {
            x: ML + 6,
            y: dy - 8,
            font: fontBold,
            size: 9,
      ...
      if (dy > 60) {
        ...
        p2.drawText("= Resultado Líquido do Período", {
          x: ML + 6,
          y: dy - 9,
          font: fontBold,
          size: 10,
    ...
    if (signErr)
      console.error("[client-report-generate] signed URL error:", JSON.stringify(signErr));
    ...
    try {
      ...
      if (exportErr) {
        console.error(
          "[client-report-generate] report_exports upsert failed:",
